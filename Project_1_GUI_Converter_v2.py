...
def urlencode():                                #URL Encode
    urleinp = txt_in.get(1.0,END)
    urlenc = urllib.parse.quote(urleinp, safe="")   #Safe Off, So "/" can also encoded
    
    txt_out.config(state=NORMAL)
    txt_out.delete(1.0,END)
    txt_out.insert(1.0,urlenc)
    txt_out.config(state=DISABLED)

def urldecode():                                #URL Decode
    urldinp = txt_in.get(1.0,END)
    urldec = urllib.parse.unquote(urldinp)

    txt_out.config(state=NORMAL)
    txt_out.delete(1.0,END)
    txt_out.insert(1.0,urldec)
    txt_out.config(state=DISABLED)
...
def decascii():                              #Decimal to ascii
    ...
    decasciilst = []
    decasciilst2 = []
    decasciilst=decascii.split()              #Split based on " ".

    for i in decasciilst:
        if i == "/":                          #Replace "/" with "_"
            decasciilst2.append('_')
        else:
            decasciilst2.append(chr(int(i)))  #Replace ASCII Character to Text.
...
def asciidec():                              #ascii to decimal
    ...
    for l in asciideclst:
        for i in l:
            asciideclst2.append(ord(i))
        asciideclst2.append("/")             #Add a "/" after completion of every word
...
def morseencode():                           #Morse Encode
    ...
    for i in moralp:                              #Character to Morse
        for j in i:
            match j:
                case "A" | "a":
                    morlst.append('01')
...
def morsedecode():                           #Morse Decode
    ...
    for i in mordelst:                           #Morse to Character
        match i:
            case '01':
                mordelst2.append('a')
...
def about():                                 #About Button
    ...
    Label(aboutwin, text="Project: Project 1 Converter (GUI Revised Version)\nVersion: 2.0\nDated On: 12 August 2024\nLast Modified : June 2025", font=("",12,""),bg="#f6e947", justify=LEFT).place(x=19,y=65)
    Button(aboutwin,text="Previous Version Link", font=("",9,""),command=webpre).place(x=360,y=200,anchor=CENTER)
    Button(aboutwin,text="This Version Link", font=("",9,""),command=webnew).place(x=360,y=240, anchor=CENTER)
...
def disable():
    ...
    R11.config(state=DISABLED, disabledforeground="#313131")
    R12.config(state=DISABLED, disabledforeground="#313131")
...
def choose():                                #Submit Button
    ...
    if choice.get() == 1 :
        ...
        Label(hk2,text="Decimal to Binary", font=("",12,"bold","underline","italic"),bg=color).place(x=225,y=50, anchor=CENTER)
        ...
        hk2.config(bg=color)
        hk2.title("Instructions")
        Label(hk2,text="Decimal to Octal", font=("",12,"bold","underline","italic"),bg=color).place(x=225,y=50., anchor=CENTER)
        ...
        hk2.config(bg=color)
        hk2.title("Instructions")
        Label(hk2,text="Decimal to HexaDecimal", font=("",12,"bold","underline","italic"),bg=color).place(x=225,y=50, anchor=CENTER)
        ...
        hk2.config(bg=color)
        hk2.title("Instructions")
        Label(hk2,text="Decimal to ASCII", font=("",12,"bold","underline","italic"),bg=color).place(x=225,y=50, anchor=CENTER)
        ...
        hk2.config(bg=color)
        hk2.title("Instructions")
        Label(hk2,text="Binary to Decimal", font=("",12,"bold","underline","italic"),bg=color).place(x=225,y=50,anchor=CENTER)
        ...
        hk2.config(bg=color)
        hk2.title("Instructions")
        Label(hk2,text="Octal to Decimal", font=("",12,"bold","underline","italic"),bg=color).place(x=225,y=50, anchor=CENTER)
        ...
        hk2.config(bg=color)
        hk2.title("Instructions")
        Label(hk2,text="HexaDecimal to Decimal", font=("",12,"bold","underline","italic"),bg=color).place(x=225,y=50, anchor=CENTER)
        ...
        hk2.config(bg=color)
        hk2.title("Instructions")
        Label(hk2,text="ASCII to Decimal", font=("",12,"bold","underline","italic"),bg=color).place(x=225,y=50, anchor=CENTER)
        ...
        hk2.config(bg=color)
        hk2.title("Instructions")
        Label(hk2,text="Morse Encode", font=("",12,"bold","underline","italic"),bg=color).place(x=225,y=50,anchor=CENTER)
        txt="Morse Encode"
        ...
        hk2.config(bg=color)
        hk2.title("Instructions")
        Label(hk2,text="Morse Decode", font=("",12,"bold","underline","italic"),bg=color).place(x=225,y=50,anchor=CENTER)
        txt="Morse Decode"
        ...
        hk2.config(bg=color)
        hk2.title("Instructions")
        Label(hk2,text="URL Encode", font=("",12,"bold","underline","italic"),bg=color).place(x=225,y=50,anchor=CENTER)
        txt="URL Encode"
        Label(hk2,text="This Mode is used to Encode URL\nto Browser readable format.\n\nInput should be in URL Format.\n(ex:https://google.com/any=Hii HK).\n\nOutput will be in Text\n(ex:https%3A%2F%2Fgoogle.com%2Fany%3DHii%20HK)", bg=color, font=("",11,"")).place(x=225,y=270,anchor=CENTER)
        ...
        hk2.config(bg=color)
        hk2.title("Instructions")
        Label(hk2,text="URL Decode", font=("",12,"bold","underline","italic"),bg=color).place(x=225,y=50,anchor=CENTER)
        txt="URL Decode"
        Label(hk2,text="This Mode is used to Decode URL\nto Human readable format.\n\nInput should be in URL Encoded format.\n(ex:https%3A%2F%2Fgoogle.com%2Fany%3DHii%20H).\n\nOutput will be in Text\n(ex:https://google.com/any=Hii HK)", bg=color, font=("",11,"")).place(x=225,y=270,anchor=CENTER)
    ...
    lab.place(x=633,y=38.5,anchor=CENTER)
...
mode_label= Label(frame, text="Modes", font=('Comic Sans MS',25,"bold","italic","underline"), bg=framecol)
mode_label.place(x=120,y=35, anchor=CENTER)

choice = IntVar()

# From Decimal Options
...
urlcode= Label(frame, text=">> URL", font=('',11,"bold"), bg=framecol)
urlcode.place(x=15, y=495)

R11 = Radiobutton(frame, text="Encode", variable=choice, value=11, bg=framecol, activebackground=framecol ,activeforeground="#000000", fg="#000000", font=("",10,""))
R11.place(x=35,y=525)

R12 = Radiobutton(frame, text="Decode", variable=choice, value=12, bg=framecol, activebackground=framecol ,activeforeground="#000000", fg="#000000", font=("",10,""))
R12.place(x=35,y=555)

#Text Input Field
txt_in= Text(hk, font=('',12,''), height=20,width=40, state=DISABLED, bg="#d8d8d8",padx=10, pady=10)
txt_in.place(x=240,y=112, anchor=NW)

#Text Output Field
txt_out= Text(hk, font=('',12,''), height=20,width=40, bg="#d8d8d8", padx=10, pady=10)
txt_out.config(state=DISABLED)
txt_out.place(x=1026,y=112, anchor=NE)

#Buttons : Proceed, Reset, Submit, Save to File, About Me

But1 = Button(frame, text="Submit", font=("",9,"bold"), command=choose)
But1.place(x=35, y= 600)

But2 = Button(frame, text="Reset",font=("",9,"bold"),command=reset, state=DISABLED)
But2.place(x=120,y=600)

But3 = Button(hk, text="Proceed",font=("",10,""),command=proceed, state=DISABLED)
But3.place(x=633,y=540, anchor=CENTER)

But4 = Button(hk, text="About Me", font=("",10,""),command=about)
But4.place(x=1026,y=620, anchor=SE)

But5 = Button(hk, text="Save to File", font=("",10,""), command=fileout, state=DISABLED)
But5.place(x=633,y=575, anchor=CENTER)

#Input/Output Field on Top of Input/Output Fields
Label(hk, text="Input", font=("",12,"bold","italic"), bg="#529eff",height=1, width=6, relief="ridge", bd=5).place(x=240,y=75,anchor='nw')
Label(hk, text="Output", font=("",12,"bold","italic"), bg="#529eff",height=1, width=6, relief="ridge", bd=5).place(x=1026,y=75,anchor='ne')

lab=Label(hk,text="HK_Project_GUI_V2_Converter", font=("",20,"bold","italic"),bg=color)
lab.place(x=633,y=38.5,anchor=CENTER)
